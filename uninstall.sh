...
#
# Membersihkan SEMUA yang dibuat oleh installer speedtest, termasuk:
#   - OoklaServer binary, properties, service
#   - ZeroTier installation, service, moon, network membership
#   - Certbot certificates (Let's Encrypt)
#   - Cloudflare credentials
#   - Cron jobs (reporting, restart, moon-updater)
#   - IP forwarding, NAT iptables rules
#   - Semua file di BASE_DIR (kecuali data.ini)
#
# HANYA MENYISAKAN: data.ini
#
# Penggunaan:
#   bash ./uninstall.sh                    # Auto-detect BASE_DIR dari CWD
#   bash ./uninstall.sh /path/ke/data.ini  # Tentukan lokasi data.ini
...
echo ""
echo "  PERINGATAN: Script ini akan menghapus SEMUA komponen"
echo "  yang diinstal oleh installer speedtest, termasuk:"
echo ""
echo "    - OoklaServer (binary, service, cron)"
echo "    - Sertifikat Let's Encrypt"
echo "    - Kredensial Cloudflare"
echo "    - ZeroTier (binary, service, network, moon)"
echo "    - Speedtest CLI"
echo "    - Semua file di BASE_DIR (kecuali data.ini)"
echo "    - Firewall rules & IP forwarding"
echo ""
echo "  HANYA data.ini YANG DISISAKAN."
echo "============================================================"
echo ""
...
# ===============================
log_step "Step 1: Menghentikan semua service yang berjalan"
...
if pgrep -x zerotier-one >/dev/null 2>&1; then
  ...
  if command -v zerotier-cli >/dev/null 2>&1; then
    NETID=$(zerotier-cli listnetworks 2>/dev/null | awk '{print $3}' | head -1 || true)
    [ -n "$NETID" ] && zerotier-cli leave "$NETID" 2>/dev/null || true
  fi
...
# ===============================
# STEP 4: REMOVE ZEROTIER
# ===============================
log_step "Step 4: Menghapus ZeroTier"
...
safe_rm "/var/lib/zerotier-one" "ZeroTier data directory"
safe_rm_file "/usr/local/bin/zt-exitnode.sh" "ZT exit node script"
safe_rm_file "/usr/local/bin/zt-moon-updater.sh" "ZT moon updater"
safe_rm_file "/var/log/zt-moon-updater.log" "ZT moon updater log"

# Package removal (best effort)
...
# ===============================
# STEP 6: REMOVE SSL & CLOUDFLARE CREDENTIALS
# ===============================
log_step "Step 6: Menghapus sertifikat & kredensial"

safe_rm_file "/etc/letsencrypt/dnscloudflare.ini" "Cloudflare credentials"

if [ -f "$CONFIG_PATH" ]; then
  DOMAIN=$(grep -E '^[[:space:]]*Domain[[:space:]]*=' "$CONFIG_PATH" 2>/dev/null | tail -1 | sed 's/.*=[[:space:]]*//' | tr -d \"\' | xargs)
...
# ===============================
# STEP 7: REMOVE IP FORWARDING & NAT
# ===============================
...
if command -v iptables >/dev/null 2>&1; then
  ...
  log_ok "iptables rules dibersihkan."
fi

# ===============================
# STEP 8: REMOVE BASE_DIR CONTENTS (except data.ini)
# ===============================
...
if command -v certbot >/dev/null 2>&1; then
  if confirm "Hapus juga certbot? (hanya jika tidak digunakan oleh aplikasi lain)"; then
    if command -v apk >/dev/null 2>&1; then
...
echo ""
echo "  Yang dihapus:"
echo "    [x] OoklaServer (binary, service, cron, properties)"
echo "    [x] ZeroTier (binary, service, data, scripts, moon)"
echo "    [x] Speedtest CLI"
echo "    [x] Sertifikat Let's Encrypt"
echo "    [x] Kredensial Cloudflare"
echo "    [x] IP forwarding & iptables NAT"
echo "    [x] Semua file di BASE_DIR (kecuali data.ini)"
echo "    [x] Semua cron jobs terkait"
echo ""
echo "  Yang disisakan:"
echo "    [i] $CONFIG_PATH"
echo ""
echo "  BASE_DIR sekarang hanya berisi data.ini:"
