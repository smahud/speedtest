...
install_zerotier() {
    ...
    if [ "${OS_FAMILY:-}" = "alpine" ]; then
        local alpine_ver=$(cut -d. -f1,2 /etc/alpine-release 2>/dev/null || echo "latest-stable")
        ...
        if apk add --no-cache zerotier-one >/dev/null 2>&1; then
            ...
            if wget -q -O "$local_bin" "$remote_bin"; then
                ...
                log_error "Gagal menginstal ZeroTier: Repository dan biner fallback tidak tersedia."
                return 1
            fi
        fi
    else
        ...
        if curl -s https://install.zerotier.com | bash >/dev/null 2>&1; then
            ...
            if command -v apt-get >/dev/null 2>&1; then
                DEBIAN_FRONTEND=noninteractive apt-get install -y zerotier-one >/dev/null 2>&1
            elif command -v dnf >/dev/null 2>&1; then
                dnf install -y zerotier-one >/dev/null 2>&1
            elif command -v yum >/dev/null 2>&1; then
                yum install -y zerotier-one >/dev/null 2>&1
            fi
            [ $? -eq 0 ] && log_ok "ZeroTier berhasil diinstal via package manager." || { log_error "Gagal menginstal ZeroTier."; return 1; }
        fi
    fi
...
verify_zerotier_service() {
    ...
    local status_output=$(zerotier-cli info 2>/dev/null || true)
...
check_authorization() {
    ...
    local network_status=$(zerotier-cli listnetworks 2>/dev/null | grep "$NETWORK_ID" || true)
